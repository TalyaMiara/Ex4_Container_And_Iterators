...
private:
    ...
    std::size_t pos = 0;       // Current position in crossView
...
public:
     ...
     */
    SideCrossOrder(const std::vector<T>& elements, std::size_t startPos = 0)
        : pos(startPos)
    {
        ...
        if (crossView.empty()) {
            throw std::invalid_argument("Cannot create SideCrossOrder with empty elements");
        } 

        // Create side-cross order
        std::size_t left = 0;
     ...
     * Compares two SideCrossOrder instances to check if they point to the same index.
     * 
